...
//            frame.append("<link rel=\"stylesheet\" href=\"video-js.min.css\">");
            ...
            frame.append("<h1 onclick='dspl()' id=\"showName\" style = \"color:red;margin-bottom:0px;text-align: center;\">点击列表↓↓↓↓播放音乐</h1>");
            frame.append("<div id='collectDiv' style='display: none;'></div>");
            //头部隐藏块
            frame.append("<div id='showView' style='display: none; margin: 30px;flex-direction: column; align-items: center; justify-content: flex-start;'>");
            frame.append("<br>");
            frame.append("<div><input type='number' id='searchInput' style='width: 150px;' placeholder='请输入官网歌曲编号:'></input>&emsp;<button onclick='searchById()'>搜索下载</button></div>");
            frame.append("<br>");
//            frame.append("<video id='my-player'  class='video-js vjs-big-play-centered'></video>");
            frame.append("<div>倍速:<label id='label' for='range'>1</label> <input id='range' type='range' value='1' min='0.1' max='2.5' step='0.1' onchange='changeV()'></div>");
            frame.append("<br>");
            frame.append("<div><input id='timeInput' type='number' value='30' min='1' max='1200'>分钟");
            frame.append("&emsp;<button onclick='timeOut()'>定时关闭</button></div>");
            frame.append("</div>");
            //进度条
            ...
            frame.append("<h1 id=\"showDownload\" style = \"display: none;text-align: center;\"></h1>");
            //弹窗
...
/*
            ...
            frame.append("window.history.pushState({},'',window.location.origin+window.location.pathname);");
            frame.append("}else{");
            ...
            frame.append("let time = getFullTime();");
            frame.append("let id = item.children[0].id;");
            frame.append("let name = item.children[0].textContent;");
            frame.append("let date = item.parentElement.previousElementSibling.innerText;");
            frame.append("collectionList = collectionList.filter(item=>{return item.id!=id});");
            frame.append("collectionList.unshift({id,name,time,date});");
            frame.append("localStorage.setItem('collectionList',JSON.stringify(collectionList));");
            frame.append("isCollection(id);}");
            frame.append("showCollection();");
            frame.append("}}");
            ...
            frame.append("collect_body += '<span>'+item.date+'&emsp;</span>';");
            ...
            frame.append("if(currentLi && currentLi.children[0].id == id){SHOW_NAME.scrollIntoView({behavior: 'smooth'});return;}");
            ...
            frame.append("SHOW_DOWNLOAD.style.setProperty('display', 'block');");
            ...
            //完整时间
            frame.append("function getFullTime(){");
            frame.append("let date=new Date(),Y=date.getFullYear(),M=date.getMonth()+1,D=date.getDate(),h=date.getHours(),m=date.getMinutes(),s=date.getSeconds();");
            frame.append("return Y+'-'+(M<10?'0'+M:M)+'-'+(D<10?'0'+D:D)+' '+(h<10?'0'+h:h)+':'+(m<10?'0'+m:m)+':'+(s<10?'0'+s:s);");
            frame.append("};");
            ...
            frame.append("}else if(inputId){");
            frame.append("let aa = document.getElementById(inputId);");
            frame.append("if(aa){aa.click();SEARCH_INPUT.value = '';return;}};");
            frame.append("");
